...
export class TelegramAlerter {
  constructor({ botToken, chatId, timeout }) {
    this.botToken = botToken;
    this.chatId = chatId;
    // For 30s server long polling, give 35s client timeout to avoid race conditions
    this.timeout = timeout || config.telegram.timeout || 35000;
    ...
    this._pollingInProgress = false; // Prevent concurrent getUpdates (causes 409)
  ...
  async evaluateAndAlert(llmProvider, delta, memory) {
    ...
    if (this._isMuted()) {
      console.log('[Telegram] Alerts muted until', formatLocalTime(this._muteUntil));
      return false;
    }
  ...
  async _pollUpdates() {
    // Prevent concurrent getUpdates calls which cause 409 Conflict
    ...
    try {
      ...
      const data = await safeFetch(`${TELEGRAM_API}/bot${this.botToken}/getUpdates?${params}`, {
      ...
      if (data.error) {
        ...
        if (data.error.includes('409') || data.error.includes('Conflict')) {
          console.warn('[Telegram] Poll conflict detected (another bot instance running): stopping polling');
          this.stopPolling();
          return;
        }
        // Silent for other expected errors unless it's not an abort
        if (!data.error.includes('aborted')) {
          console.error('[Telegram] Poll error:', data.error);
        }
        return;
      }
  ...
  async _handleMessage(msg) {
    ...
    if (command === '/mute') {
      ...
      await this.sendMessage(
         `🔇 Alerts muted for ${hours}h — until ${formatLocalTime(this._muteUntil)} ${config.timezone || 'UTC'}\nUse /unmute to resume.`,
        { chatId: replyChatId, replyToMessageId: msg.message_id }
      );
      return;
    }
    ...
    if (command === '/alerts') {
      ...
      const lines = recent.map(a =>
         `${TIER_CONFIG[a.tier]?.emoji || '⚪'} ${a.tier} — ${formatLocalTime(a.timestamp)}`
      );
      await this.sendMessage(
  ...
  async _initializeBotCommands() {
    ...
    const botCommands = Object.entries(COMMANDS).map(([command, description]) => {
      ...
      const localizedDesc = t(`bot.commands.${cmdKey}`) || description;
      return {
        command: cmdKey,
        description: localizedDesc.substring(0, 256),
      };
    });
  ...
  _buildSignalContext(signals, delta) {
    ...
    if (conflictSignals.length > 0) {
      sections.push(`${t('layers.conflictEvents').toUpperCase()}:\n` + conflictSignals.map(s =>
        `  ${s.label}: ${s.from} → ${s.to} (${s.direction})`
      ).join('\n'));
    }

    if (otherSignals.length > 0) {
      sections.push(`${t('panels.signalCore').toUpperCase()}:\n` + otherSignals.map(s =>
  ...
  _formatTieredAlert(evaluation, delta, tier) {
    ...
    const tierLabel = t(`bot.alertTiers.${tier.toLowerCase()}`) || tc.label;
    const confidenceLabel = t(`alerts.confidence.${evaluation.confidence?.toLowerCase()}`) || evaluation.confidence;
    ...
    const lines = [
      ...
      evaluation.reason,
      ``,
      `${t('alerts.fields.confidence')}: ${confidenceEmoji} ${confidenceLabel}`,
      `${t('alerts.fields.direction')}: ${t(`delta.${delta.summary.direction}`).toUpperCase()}`,
    ];

    if (evaluation.crossCorrelation) {
      lines.push(`${t('alerts.fields.crossCorrelation')}: ${evaluation.crossCorrelation}`);
    }

    if (evaluation.actionable && evaluation.actionable !== 'Monitor') {
      lines.push(``, `💡 *${t('alerts.fields.action')}:* ${evaluation.actionable}`);
    }
    ...
    let formattedDate;
    if (config.timezone) {
      formattedDate = new Date().toLocaleString(currentLanguage, { 
    ...
    lines.push('', `_${formattedDate} ${timezoneLabel}_`);
