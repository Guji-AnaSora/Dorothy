...
export class DiscordAlerter {
  ...
  async _handleCommand(interaction) {
    ...
    if (name === 'mute') {
      ...
      await interaction.reply({
         embeds: [this._embed('Alerts Muted', `Alerts silenced for ${hours}h — until ${formatLocalTime(this._muteUntil)} ${config.timezone || 'UTC'}.\nUse \`/unmute\` to resume.`, 0x95A5A6)],
        ephemeral: true,
      });
      return;
    }
    ...
    if (name === 'alerts') {
      ...
      const lines = recent.map(a =>
         `${tierEmoji[a.tier] || '⚪'} **${a.tier}** — ${formatLocalTime(a.timestamp)}`
      );
      await interaction.reply({
  ...
  async evaluateAndAlert(llmProvider, delta, memory) {
    ...
    if (this._isMuted()) {
      console.log('[Discord] Alerts muted until', formatLocalTime(this._muteUntil));
      return false;
    }
  ...
  _buildAlertEmbed(evaluation, delta, tier) {
    ...
    const footerText = `Crucix Intelligence · ${formattedDate} ${timezoneLabel}`;
