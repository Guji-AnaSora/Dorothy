...
// Get current language from environment every time it's accessed
export const currentLanguage = getLanguage();

/**
 * Check if content translation is needed for current language
 * @returns {boolean} True if translation is needed (only zh needs translation of English content)
 */
...
// Get the language again at log time to ensure we log the correct value
// even if environment variables changed after module load
console.log(`[i18n] Language: ${getLanguage()}`);
